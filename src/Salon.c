...
eSalon* salonNuevo(void)
{
	eSalon* pSalon = malloc(sizeof(eArcade));
	if(pSalon!=NULL)
	{
		pSalon->isEmpty=0;
		pSalon->idSalon=0;
...
int sal_cargarSalon(eSalon* pSalon,int *bufferIdSalon)
 {
	...
	if (pSalon != NULL)
		{
				...
				utn_obtenerEnteroConReintentosMinMax(&tipo,"Por favor ingrese el tipo de Salon:  1=SHOPPING - 2=LOCAL: ","Dato invalido.\n", 1, 2, 3) == 0 )
			{
					...
					*bufferIdSalon=idSalon;
					status = 0;
					sal_adhSalon(pSalon, TAM_DATOSALON,idSalon,nombre, direccion,tipo);
...
int sal_adhSalon(eSalon* unSalon, int len, int idSalon,char nombre[],char direccion[],int tipo)
{

	int bufferindice;

	if(unSalon != NULL && len>0 && nombre!= NULL && direccion!=NULL)
	{
		unSalon->idSalon=idSalon;
		strncpy(unSalon->nombre,nombre,sizeof(unSalon->nombre));
		strncpy(unSalon->direccion,direccion,sizeof(unSalon->direccion));
		unSalon->tipo=tipo;
		return 0;
...
int sal_buscarLibreSalon(eSalon* listSalon[], int len, int* posicionLibre)
{
	int index=0;
	int estado=-1;
