...
int arc_iniArcade(eArcade* listArcade[], int len)
{
	...
	if (listArcade != NULL && len > 0)
	{
		...
		{
			listArcade[indice] = NULL;
		}
	}
	return status;
}
//int display_initializeList (Display** displaysList, int listLen)
...
int arc_cargarArcade(eArcade* listArcade[])
 {
	...
	if (listArcade != NULL)
		{
			if (utn_getDescripcion(nombreDelJuego, TAM_DATOARCADE, "Por favor ingrese el nombre del Juego: ", "Dato invalido.\n", 3)== 0 &&
				...
				bufferIdArcade = createIdArcade();
					status = 0;
					arc_adhArcade(posicionLibre, TAM_ARCADE, bufferIdArcade,nombreDelJuego, nacionalidad,tipoDeSonido, cantidadDeJugadores, capacidadMaxFichas);
...
int arc_adhArcade(eArcade* listArcade, int len, int idArcade,char nombreDelJuego[],char nacionalidad[],int tipoDeSonido,int cantidadDeJugadores, int capacidadMaxFichas,int idSalon)
{
	...
	int bufferindice;
	{
		listArcade->idArcade=idArcade;
		listArcade->idSalon=idSalon;
		strncpy(listArcade->nombreDelJuego,nombreDelJuego,sizeof(listArcade->nombreDelJuego));
		strncpy(listArcade->nacionalidad,nacionalidad,sizeof(listArcade->nacionalidad));
		listArcade->tipoDeSonido=tipoDeSonido;
		listArcade->cantidadDeJugadores=cantidadDeJugadores;
		listArcade->capacidadMaxFichas=capacidadMaxFichas;
		return 0;
...
//int display_findAnEmptyPosition(Display displaysList[], int listLen, int* pEmptyPosition)

int arc_buscarLibreArcade(eArcade* listArcade[], int len, int* posicionLibre)
{
	int i=0;
	int estado=-1;
...
int arc_buscarPorId(eArcade* listArcade[], int len,int idArcade, int* pIndexPorId)
{

		int indice;
		int estado = -1;
		if (listArcade != NULL && len > 0 && idArcade > 0)
		{
			for (indice = 0; indice < len; indice++)
			{
				if (listArcade[indice]==NULL)
				{
...
int arc_borrarArcade(eArcade* listArcade[], int len, int idArcade)
{
	...
	int opcion;
	int opcionDos;
	arc_buscarPorId(listArcade,len,idArcade,bufferindice);

		if(bufferindice>=0 && listArcade[bufferindice]==NULL)
		{
